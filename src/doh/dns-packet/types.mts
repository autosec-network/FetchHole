...
export class Types {
	public static toString(type: number): string {
		return DnsRecordType[type] ?? `UNKNOWN_${type}`;
	}

	public static toType(name: string): number {
		const upperCaseName = name.toUpperCase();

		if (upperCaseName in DnsRecordType) {
			return DnsRecordType[upperCaseName as keyof typeof DnsRecordType];
		}
